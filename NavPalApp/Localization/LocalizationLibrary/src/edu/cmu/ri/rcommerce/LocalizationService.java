...
/**
 * This is the primary interface with which the localization code is utilized.
 * 
 * 
 */
...
public class LocalizationService extends Service {
	...
	private float ORIENTATION_OFFSET = 2.2f; //in radians
	private float WALKING_VELOCITY = 1.3f; //in meters/s
	private final int BROADCAST_RATE = 2000; //in milliseconds
			...
			margLocalization = new PedestrianLocalization_Gyro(new edu.cmu.ri.rcommerce.LocationListener() {
				...
				public void broadcastLocationUpdate(double x, double y, double theta, long time, double velocity) {
					...
					if (updater != null && !wifiConvergenceWait) {
						...
						if(System.currentTimeMillis() - lastBroadcastTime > BROADCAST_RATE) {
							broadcast();
		...
		pfThread = new Thread(new Runnable() {
			public void run() {
				while (flag) {
					...
					broadcast();
					Log.i("PROFILING_LOCALIZATION", "Time since last full location broadcast: " + (System.nanoTime() - profilingTime) / 1.0e9f + " seconds");
					lastBroadcastTime = System.currentTimeMillis();
