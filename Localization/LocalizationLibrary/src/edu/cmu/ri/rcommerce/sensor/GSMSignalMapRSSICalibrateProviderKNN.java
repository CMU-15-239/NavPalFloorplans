 ...
 * @author Evan
 */
public class GSMSignalMapRSSICalibrateProviderKNN implements RSSICalibrateProvider<Particle2D> {
	List<CorrelatedSignalValues> data;
	int k;
	public GSMSignalMapRSSICalibrateProviderKNN(BufferedReader input,int k) throws IOException {
	...
	public List<RSSIReading> getExpectedReadings(Particle2D particle) {
		...
		for (int i = 0 ; i<k ; i++)
			neighbors.add(data.get(i).toRSSIReadingGSM());
